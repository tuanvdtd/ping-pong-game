...
/* Definitions for hapticQueue */
osMessageQueueId_t hapticQueueHandle;
const osMessageQueueAttr_t hapticQueue_attributes = {
  .name = "hapticQueue"
};
/* USER CODE BEGIN PV */
uint32_t dual_adc_buffer;
uint8_t isRevD = 0; /* Applicable only for STM32F429I DISCOVERY REVD and above */
/*
 * Cortex-M4 reads and writes aligned 32-bit words atomically. HardwareTask
 * publishes both slider values together, so the GUI never receives a mixed
...
static void DebugUart_WriteText(const char* text);
static void DebugUart_LogAdc(HAL_StatusTypeDef status,
                             uint16_t raw1,
                             uint16_t raw2,
                             uint16_t player1,
                             uint16_t player2);
...
static void DebugUart_WriteChar(char character)
{
...
static void DebugUart_WriteUInt32(uint32_t value)
{
  char digits[10];
  uint32_t count = 0U;
  ...
  {
    --count;
    DebugUart_WriteChar(digits[count]);
  }
}

static void DebugUart_WriteHex32(uint32_t value)
{
  static const char hexDigits[] = "0123456789ABCDEF";
  int32_t shift;

  DebugUart_WriteText("0x");
  for (shift = 28; shift >= 0; shift -= 4)
  {
    DebugUart_WriteChar(hexDigits[(value >> (uint32_t)shift) & 0x0FU]);
  }
}
...
static void DebugUart_Init(void)
{
  ...
  USART1->CR2 = 0U;
  USART1->CR3 = 0U;
  peripheralClock = HAL_RCC_GetPCLK2Freq();
  USART1->BRR = (peripheralClock + (115200U / 2U)) / 115200U;
  USART1->CR1 = USART_CR1_TE | USART_CR1_UE;

  DebugUart_WriteText("\r\n[PINGPONG] UART ADC debug ready: 115200 8N1\r\n");
}
...
static void DebugUart_LogAdc(HAL_StatusTypeDef status,
                             ...
                             uint16_t player2)
{
  DebugUart_WriteText("[ADC] status=");
  DebugUart_WriteText((status == HAL_OK) ? "OK" : "ERROR");
  DebugUart_WriteText(" raw1=");
  DebugUart_WriteUInt32(raw1);
  DebugUart_WriteText(" raw2=");
  DebugUart_WriteUInt32(raw2);
  DebugUart_WriteText(" p1=");
  DebugUart_WriteUInt32(player1);
  DebugUart_WriteText(" p2=");
  DebugUart_WriteUInt32(player2);
  DebugUart_WriteText(" adc_error=");
  DebugUart_WriteHex32(HAL_ADC_GetError(&hadc1));
  DebugUart_WriteText("\r\n");
}
...
static HAL_StatusTypeDef ReadAdcChannel(uint32_t channel, uint16_t* value)
{
  ...
  status = HAL_ADC_ConfigChannel(&hadc1, &channelConfig);
  if (status != HAL_OK)
  {
    return status;
  }
  ...
  /*
   * Discard the first conversion after switching channels so the ADC sample
   * capacitor can settle, then average eight independent conversions.
   */
  for (sampleIndex = 0U; sampleIndex < 9U; ++sampleIndex)
  {
                                          ...
                                          uint16_t* player2Raw)
{
  ...
  if ((player1Raw == NULL) || (player2Raw == NULL))
  {
    return HAL_ERROR;
  }

  status = ReadAdcChannel(ADC_CHANNEL_5, player1Raw);
  if (status != HAL_OK)
  {
    return status;
  }
...
int main(void)
{
  ...
  /* Create the queue(s) */
  /* creation of hapticQueue */
  hapticQueueHandle = osMessageQueueNew (4, sizeof(uint8_t), &hapticQueue_attributes);
...
static void MX_ADC1_Init(void)
{
  ...
  */
  ...
  /*
   * Keep the runtime input path in single-channel mode even if CubeMX later
   * regenerates the two-rank ADC configuration stored in the .ioc file.
   */
  /* Override disabled to allow ADC1 Scan Mode & DMA Continuous Requests */
  /*
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : SPI5_NCS_Pin MOTOR_PLAYER_Pin MOTOR_CPU_Pin */
  GPIO_InitStruct.Pin = SPI5_NCS_Pin|MOTOR_PLAYER_Pin|MOTOR_CPU_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
...
uint8_t AppBackend_GetLatestInput(uint16_t* player1, uint16_t* player2)
{
  ...
  message = latestInputMessage;
  *player1 = (uint16_t)(message & 0xFFFFU);
  *player2 = (uint16_t)(message >> 16);
...
void AppBackend_SendHaptic(uint8_t command)
{
  ...
  if ((hapticQueueHandle == NULL) ||
      (command < APP_HAPTIC_PLAYER_1) ||
      (command > APP_HAPTIC_STOP_ALL))
  {
    return;
  }

  if (command == APP_HAPTIC_STOP_ALL)
  {
    (void)osMessageQueueReset(hapticQueueHandle);
  }

  if (osMessageQueuePut(hapticQueueHandle, &command, 0U, 0U) != osOK)
  {
    (void)osMessageQueueGet(hapticQueueHandle, &discarded, NULL, 0U);
    (void)osMessageQueuePut(hapticQueueHandle, &command, 0U, 0U);
  }
}
...
{
  /* USER CODE BEGIN 5 */
  int32_t filteredPlayer1;
  int32_t filteredPlayer2;
  ...
  uint16_t player2Raw = 2048U;

  (void)argument;

  HAL_GPIO_WritePin(MOTOR_PLAYER_GPIO_Port, MOTOR_PLAYER_Pin, MOTOR_OFF_STATE);
  HAL_GPIO_WritePin(MOTOR_CPU_GPIO_Port, MOTOR_CPU_Pin, MOTOR_OFF_STATE);
  ...
  for(;;)
  {
    /* Extract 16-bit P1 (ADC1 - PA5) and 16-bit P2 (ADC2 - PC3) from 32-bit dual buffer */
    player1Raw = (uint16_t)(dual_adc_buffer & 0xFFFF);
    player2Raw = (uint16_t)((dual_adc_buffer >> 16) & 0xFFFF);
    adcStatus = HAL_OK;

    filteredPlayer1 += (((int32_t)player1Raw << 4) - filteredPlayer1) / 4;
    filteredPlayer2 += (((int32_t)player2Raw << 4) - filteredPlayer2) / 4;

    player1 = (uint16_t)(((uint32_t)(filteredPlayer1 >> 4) * 1000U) / 4095U);
    ...
    */
    ...
    {
      DebugUart_LogAdc(adcStatus, player1Raw, player2Raw, player1, player2);
      debugDeadline = now + 500U;
    }

    while (osMessageQueueGet(hapticQueueHandle, &hapticCommand, NULL, 0U) == osOK)
    {
      if (hapticCommand == APP_HAPTIC_PLAYER_1)
      {
        DebugUart_WriteText("[HAPTIC] player1 ON\r\n");
        HAL_GPIO_WritePin(MOTOR_PLAYER_GPIO_Port, MOTOR_PLAYER_Pin, MOTOR_ON_STATE);
        playerDeadline = now + 80U;
        playerMotorActive = 1U;
      }
      else if (hapticCommand == APP_HAPTIC_PLAYER_2)
      {
        DebugUart_WriteText("[HAPTIC] player2 ON\r\n");
        HAL_GPIO_WritePin(MOTOR_CPU_GPIO_Port, MOTOR_CPU_Pin, MOTOR_ON_STATE);
        cpuDeadline = now + 80U;
        cpuMotorActive = 1U;
      }
      else if (hapticCommand == APP_HAPTIC_STOP_ALL)
      {
        DebugUart_WriteText("[HAPTIC] all OFF\r\n");
        HAL_GPIO_WritePin(MOTOR_PLAYER_GPIO_Port, MOTOR_PLAYER_Pin, MOTOR_OFF_STATE);
        HAL_GPIO_WritePin(MOTOR_CPU_GPIO_Port, MOTOR_CPU_Pin, MOTOR_OFF_STATE);
        playerMotorActive = 0U;
        cpuMotorActive = 0U;
      }
      else
      {
    ...
    if ((playerMotorActive != 0U) &&
        ((int32_t)(now - playerDeadline) >= 0))
    {
      HAL_GPIO_WritePin(MOTOR_PLAYER_GPIO_Port, MOTOR_PLAYER_Pin, MOTOR_OFF_STATE);
      playerMotorActive = 0U;
    }

    if ((cpuMotorActive != 0U) &&
        ((int32_t)(now - cpuDeadline) >= 0))
    {
      HAL_GPIO_WritePin(MOTOR_CPU_GPIO_Port, MOTOR_CPU_Pin, MOTOR_OFF_STATE);
      cpuMotorActive = 0U;
    }
