...
- Phần giao diện sử dụng TouchGFX với hai màn hình chính: màn hình chọn chế độ và màn hình chơi.
- Phần xử lý game nằm trong `GameEngine`, viết bằng C++ thuần, không phụ thuộc trực tiếp vào HAL, FreeRTOS hay widget TouchGFX.
- Phần đọc phần cứng nằm trong `HardwareTask`, chạy chu kỳ 10 ms để đọc hai kênh ADC và xử lý haptic motor.
- `Model` và `Presenter` của TouchGFX đóng vai trò cầu nối giữa backend C và giao diện C++.
- Luật chơi dùng sudden-death: bên nào để bóng lọt hoàn toàn qua biên ngang trước sẽ thua ngay.

__Sản phẩm__:

1. Màn hình chọn chế độ `VS_CPU` hoặc `TWO_PLAYERS`.
2. Màn hình game Ping Pong dọc với sân chơi, hai paddle, bóng, nút Home, Pause và Continue.
...
5. Điều khiển paddle bằng hai biến trở trượt đọc qua ADC1.
6. Phản hồi rung bằng hai motor: paddle nào đỡ bóng thì motor tương ứng rung trong khoảng 80 ms.
7. Popup kết quả hiển thị người thắng và nút `Chơi lại`.
8. Nút USER PA0 trên board có thể cấu hình để Pause hoặc quay về Home.
...
| Input người chơi | 2 biến trở trượt nối vào ADC1 |
| Phản hồi | 2 module motor rung điều khiển bằng GPIO |
| Nút có sẵn | USER button PA0, active-high |
...
| Biến trở Player 2 | OTA | PC3 / ADC1_IN13 / P1 pin 15 | Điều khiển paddle trên trong `TWO_PLAYERS` |
...
| Chân | Chức năng |
|---|---|
| PA0 | USER button, active-high, debounce 30 ms |
...
- Bóng vượt hoàn toàn qua cạnh trên: Player 1 thắng.
- Bóng vượt hoàn toàn qua cạnh dưới: Player 2 hoặc CPU thắng.
- Không dùng điểm số nhiều round, mỗi trận kết thúc ngay khi có một bên miss bóng.
...
- Đã có backend đọc hai kênh ADC PA5 và PC3.
- Đã có lọc EMA và chuẩn hóa input về 0..1000.
- Đã có haptic queue để rung motor PC8 hoặc PC11.
- Đã có debounce cho nút USER PA0.
...
| 3 | Chế độ chơi với CPU | `docs/images/vs-cpu-gameplay.png` |
| 4 | Chế độ hai người chơi | `docs/images/two-player-gameplay.png` |
...
- Hiện tại luật chơi là sudden-death, chưa có hệ thống điểm nhiều round.
- CPU chỉ có một mức độ khó, có thể bổ sung nhiều cấp độ phản xạ.
- Chưa có hiệu ứng âm thanh.
- Chưa có lưu lịch sử trận đấu.
- Có thể bổ sung màn hình hướng dẫn đấu dây và hiệu chỉnh biến trở.
- Có thể thêm bước calibration để lưu min/max ADC thực tế cho từng biến trở.
- Có thể cải thiện giao diện popup với nút Home riêng trong popup.
